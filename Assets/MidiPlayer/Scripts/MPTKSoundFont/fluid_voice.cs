    ...
    public partial class fluid_voice
    {
        ...
        static int[] list_of_weakgenerators_to_initialize =
     {
        ...
        public void fluid_voice_update_param(int igen)
        {
            ...
            switch (igen)
            {
                ...
                case (int)fluid_gen_type.GEN_ATTENUATION:
                    ...
                    attenuation = attenuation < 0.0f ? 0.0f : attenuation > 14440.0f ? 1440.0f : attenuation;
                ...
                case (int)fluid_gen_type.GEN_FINETUNE:
                    ...
                    pitch =
                        (gens[(int)fluid_gen_type.GEN_PITCH].Val + gens[(int)fluid_gen_type.GEN_PITCH].Mod /*+ gens[(int)fluid_gen_type.GEN_PITCH].nrpn*/)
                ...
                case (int)fluid_gen_type.GEN_REVERBSEND:
                    ...
                    reverb_send = (gens[igen].Val + gens[igen].Mod /*+ gens[igen].nrpn*/) / 1000f;
                ...
                case (int)fluid_gen_type.GEN_OVERRIDEROOTKEY:
                    ...
                    if (gens[igen].Val > -1)
                    {
                        ...
                        root_pitch = (float)gens[igen].Val * 100.0f - sample.PitchAdj;
                    }
                    else
                    {
                ...
                case (int)fluid_gen_type.GEN_FILTERQ:
                    ...
                    q_dB = (gens[igen].Val + gens[igen].Mod /*+ gens[igen].nrpn*/) / 10f;
                ...
                case (int)fluid_gen_type.GEN_MODLFOTOPITCH:
                    modlfo_to_pitch = (float)(gens[igen].Val + gens[igen].Mod) /*+ gens[igen].nrpn*/;
                    modlfo_to_pitch = modlfo_to_pitch < -12000f ? -12000f : modlfo_to_pitch > 12000f ? 12000f : modlfo_to_pitch;
                    break;

                case (int)fluid_gen_type.GEN_MODLFOTOVOL:
                    modlfo_to_vol = (float)(gens[igen].Val + gens[igen].Mod)/*+ gens[igen].nrpn*/;
                    modlfo_to_vol = modlfo_to_vol < -960f ? -960f : modlfo_to_vol > 960f ? 960f : modlfo_to_vol;
                ...
                case (int)fluid_gen_type.GEN_MODLFOTOFILTERFC:
                    modlfo_to_fc = (float)(gens[igen].Val + gens[igen].Mod)/*+ gens[igen].nrpn*/;
                    modlfo_to_fc = modlfo_to_fc < -12000f ? -12000f : modlfo_to_fc > 12000f ? 12000f : modlfo_to_fc;
                    break;

                case (int)fluid_gen_type.GEN_MODLFODELAY:
                    {
                        float x = gens[igen].Val + gens[igen].Mod /*+ gens[igen].nrpn*/;
                        x = x < -12000f ? -12000f : x > 5000f ? 5000f : x;
                ...
                case (int)fluid_gen_type.GEN_MODLFOFREQ:
                    {
                        ...
                        float x = gens[igen].Val + gens[igen].Mod /*+ gens[igen].nrpn*/;
                        x = x < -16000.0f ? -16000.0f : x > 4500.0f ? 4500.0f : x;
                ...
                case (int)fluid_gen_type.GEN_VIBLFOTOPITCH:
                    viblfo_to_pitch = (float)(gens[igen].Val + gens[igen].Mod)/*+ gens[igen].nrpn*/;
                    viblfo_to_pitch = viblfo_to_pitch < -12000f ? -12000f : viblfo_to_pitch > 12000f ? 12000f : viblfo_to_pitch;
                ...
                case (int)fluid_gen_type.GEN_MODENVTOPITCH:
                    ...
                    modenv_to_pitch = modenv_to_pitch < -12000.0f ? -12000.0f : modenv_to_pitch > 12000.0f ? 12000.0f : modenv_to_pitch;
                    break;
                ...
                case (int)fluid_gen_type.GEN_MODENVTOFILTERFC:
                    ...
                    modenv_to_fc = modenv_to_fc < -12000.0f ? -12000.0f : modenv_to_fc > 12000.0f ? 12000.0f : modenv_to_fc;
                    break;
                ...
                case (int)fluid_gen_type.GEN_VOLENVATTACK:               /* SF2.01 section 8.1.3 # 34 */
                    {
                        float x = gens[igen].Val + gens[igen].Mod /*+ gens[igen].nrpn*/;
                        x = x < -12000f ? -12000f : x > 8000f ? 8000f : x;
                ...
                case (int)fluid_gen_type.GEN_KEYTOVOLENVDECAY:          /* SF2.01 section 8.1.3 # 40 */
                    {
                        float y = 1f - 0.001f * (gens[(int)fluid_gen_type.GEN_VOLENVSUSTAIN].Val + gens[(int)fluid_gen_type.GEN_VOLENVSUSTAIN].Mod/* + gens[(int)fluid_gen_type.GEN_VOLENVSUSTAIN].nrpn*/);
                        y = y < 0f ? 0f : y > 1f ? 1f : y;
                ...
                case (int)fluid_gen_type.GEN_VOLENVRELEASE:             /* SF2.01 section 8.1.3 # 38 */
                    {
                        float x = gens[igen].Val + gens[igen].Mod /*+ gens[igen].nrpn*/;
                        x = x < FLUID_MIN_VOLENVRELEASE ? FLUID_MIN_VOLENVRELEASE : x > 8000.0f ? 8000.0f : x;
                        uint count;
                        if (synth.MPTK_CorePlayer)
                        {
                            //NUM_BUFFERS_RELEASE
                ...
                case (int)fluid_gen_type.GEN_KEYTOMODENVDECAY:          /* SF2.01 section 8.1.3 # 40 */
                    {
                        ...
                        float y = 1f - 0.001f * (gens[(int)fluid_gen_type.GEN_MODENVSUSTAIN].Val + gens[(int)fluid_gen_type.GEN_MODENVSUSTAIN].Mod/* + gens[(int)fluid_gen_type.GEN_VOLENVSUSTAIN].nrpn*/);
                        y = y < 0f ? 0f : y > 1f ? 1f : y;
                ...
                case (int)fluid_gen_type.GEN_MODENVRELEASE:             /* SF2.01 section 8.1.3 # 30 */
                    {
                        float x = gens[igen].Val + gens[igen].Mod /*+ gens[igen].nrpn*/;
                        x = x < -12000f ? -12000f : x > 8000.0f ? 8000.0f : x;
                        uint count;
                        if (synth.MPTK_CorePlayer)
                        {
                            //NUM_BUFFERS_RELEASE
        ...
        uint calculate_hold_decay_buffers(int gen_base, int gen_key2base, bool is_decay)
        {
             ...
             */
            float timecents = (gens[gen_base].Val + gens[gen_base].Mod /*+ gens[gen_base].nrpn*/) +
                  (gens[gen_key2base].Val + gens[gen_key2base].Mod /*+ gens[gen_key2base].nrpn*/) * (60f - key);
...
#define fluid_phase_set_int(a, b)    ((a) = ((unsigned long long)(b)) << 32)
        ...
        public int fluid_voice_kill_excl()
        {
            ...
            if (synth.VerboseSynth) DebugVolEnv("Kill Exclusif");
