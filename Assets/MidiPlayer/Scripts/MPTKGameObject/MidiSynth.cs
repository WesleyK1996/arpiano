    ...
    public partial class MidiSynth : MonoBehaviour
    {
        ...
        public void Start()
        {
            ...
            try
            {
                ...
                Timing.RunCoroutine(ThreadLeanStartAudio(CoreAudioSource));
        ...
        /// </summary>
        /// <param name="destroyAudioSource">Destroy also audioSource (default:false). Apply only for non Core mode</param>
        ///! @code
...
#endif
            MPTK_ResetStat();
            //Debug.Log(DateTime.Now + " >>> ThreadClearAllSound");
            if (MPTK_CorePlayer)
            {
                if (SpatialSynths != null && dedicatedChannel == -1) // apply only for the MidiSynth reader
                {
                    foreach (MidiFilePlayer mfp in SpatialSynths)
                        if (mfp.QueueSynthCommand != null)
                            mfp.QueueSynthCommand.Enqueue(new SynthCommand() { Command = SynthCommand.enCmd.NoteOffAll });
        ...
        /// That could take some seconds because release time for sample need to be played.
        /// Therefor the method exit after a timeout of 3 seconds.
        /// </summary>
        /// <returns></returns>
        public void MPTK_WaitAllNotesOff()
        {
            Timing.WaitUntilDone(Timing.RunCoroutine(ThreadWaitAllStop()), false);
        }
        ...
        public IEnumerator<float> ThreadWaitAllStop()
        {
            int count = 999;
            ...
            //Debug.Log("ThreadWaitAllStop end - " + start + " count:" + count);
            yield return 0;
        ...
        /// <summary>
        /// Get count of notes played since the beguinning of the Midi.
        /// </summary>
        /// <param name="channel">must be between 0 and 15</param>
        ...
        public fluid_voice fluid_synth_alloc_voice(HiSample hiSample, int chan, int key, int vel)
        {
            ...
            if (voice == null)
            {
                if (VerboseVoice) Debug.Log($"Voice {fluid_voice.LastId} - Create - Sample:'{hiSample.Name}' Rate:{hiSample.SampleRate} hz");
                    ...
                    if (preset_zone.Index >= 0)
                    {
                        ...
                        foreach (HiZone inst_zone in inst.Zone)
                        {
                                ...
                                (inst_zone.VelHi >= vel))
                            {
                                ...
                                if (MPTK_LogWave)
                                    Debug.LogFormat("NoteOn [C:{0:00} B:{1:000} P:{2:000}]\t{3,-21}\tKey:{4,-3}\tVel:{5,-3}\tDuration:{6:0.000}\tInstr:{7,-21}\t\tSample:{8,-21}\tAtt:{9:0.00}\tPan:{10:0.00}",
        ...
        /// </summary>
        /// <param name="midievent"></param>
        protected void PlayEvent(MPTKEvent midievent, bool playNoteOff = true)
        {
                ...
                if (MPTK_DirectSendToPlayer)
                {
                    foreach (MPTKEvent midievent in midievents)
                    {
                        try
                        {
                            ...
                            {
                                SpatialSynths[midievent.Channel].PlayEvent((MPTKEvent)midievent/*.Clone()*/, false);
                            }
                            else
#endif
